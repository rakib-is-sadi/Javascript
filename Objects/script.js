...
let account = {
  ...
  withdraw(amount) {
    if (amount > this.balance) {
      console.log(
        `Insufficient Balance! You can withdraw maximum ${this.balance} TK.`,
      );
    } else {
      this.balance -= amount;
      console.log(
        `Withdraw: ${amount} TK. Remaining Balance: ${this.balance} TK.`,
      );
    }
  },
  ...
  checkBalance() {
    console.log(
      `Your name is ${this.holderName} and your current balance is ${this.balance} TK.`,
    );
  },
};
...
let student = {
  ...
  showInfo() {
    console.log(
      `${this.name} lives in ${this.details.city} and got ${this.marks.math} in Math and ${this.marks.english} in English`,
    );
  },
};
...
function showProducts(list) {
  for (let data of list) {
    console.log(`Product Name: ${data.name} & Price: ${data.price} TK.`);
  }
}
showProducts(products);

function filterByPrice(list, sort) {
  let filterPrice = [];
  for (let data of list) {
    if (data.price >= sort) {
      filterPrice.push(data);
    }
  }
  console.log(filterPrice);
}
filterByPrice(products, 500);

function discountCalc(list) {
  let discountPrice = list.map((item) => item.price - item.price * 0.1);
  console.log(discountPrice);
}
discountCalc(products);

function totalCost(list) {
  let total = list.reduce((acc, crr) => acc + crr.price * crr.stock, 0);
  console.log(total);
}
totalCost(products);

function stockCheck(list, product) {
  let findStock = list.find((item) => item.name === product);
...
stockCheck(products, "Gaming Headphone");

const students = [
  { id: 1, name: "Rakib", presentDays: 28, totalClasses: 30 },
  { id: 2, name: "Sany", presentDays: 18, totalClasses: 30 },
  { id: 3, name: "Arif", presentDays: 10, totalClasses: 30 },
  { id: 4, name: "Sadi", presentDays: 23, totalClasses: 30 },
  { id: 5, name: "Rony", presentDays: 5, totalClasses: 30 },
];

function checkExamEligibility(list, name) {
  let studentData = list.find((item) => item.name === name);
  let percentage = (studentData.presentDays / studentData.totalClasses) * 100;
  if (!percentage) {
    return;
  } else if (percentage >= 75) {
    console.log(`Name: ${studentData.name} - Exam Allowed.`);
  } else if (percentage >= 50 && percentage <= 74) {
    console.log(`Name: ${studentData.name} - Exam Allowed with FIne.`);
  } else {
    console.log(`Name: ${studentData.name} - Exam Not Allowed`);
  }
}

checkExamEligibility(students, "Rakib");


function totalPercentage(list) {
  let alldetails = list.map(item => (item.presentDays / item.totalClasses) * 100);
  console.log(alldetails);
    
}

totalPercentage(students);